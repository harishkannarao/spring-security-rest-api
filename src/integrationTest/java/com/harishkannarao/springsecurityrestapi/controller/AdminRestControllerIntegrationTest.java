...
public class AdminRestControllerIntegrationTest extends AbstractBaseIntegrationTestProfile {
    ...
    @Test
    public void test_getSensitiveData_returns200_forAdminUser() {
        HttpHeaders requestHeaders = new HttpHeaders();
        requestHeaders.setBearerAuth("admin-token");
        HttpEntity<Void> requestEntity = new HttpEntity<>(requestHeaders);
        ResponseEntity<UserData> result = testRestTemplate
                .exchange("/admin/get-sensitive-data/{username}", HttpMethod.GET, requestEntity, UserData.class, Map.of("username", "user-name-1"));
    ...
    @Test
    public void test_getSensitiveData_returns400_forNonExistentUsername_forAdminUser() {
        HttpHeaders requestHeaders = new HttpHeaders();
        requestHeaders.setBearerAuth("admin-token");
        HttpEntity<Void> requestEntity = new HttpEntity<>(requestHeaders);
        ResponseEntity<Void> result = testRestTemplate
    ...
    @Test
    public void test_getSensitiveData_returns403_forNonAdminUser() {
        HttpHeaders requestHeaders = new HttpHeaders();
        requestHeaders.setBearerAuth("user-token");
        HttpEntity<Void> requestEntity = new HttpEntity<>(requestHeaders);
        ResponseEntity<Void> result = testRestTemplate
